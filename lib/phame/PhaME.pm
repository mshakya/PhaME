...
"\n$list not found.\nChange control file to run the entire pipeline.\n";
            ...
            while (<LIST>) {
                ...
                if (/(.+)_p|sread/) {
                    ...
                    $gapsfile = glob "$gapdir/$pref_name\_$1\_$pref_name*.gaps";
                }
...
sub codingRegions {
    ...
    while (<IN>) {
        ...
        if ( !/^#/ ) {
            my (
                $chr_name, $source, $method, $start, $stop,
            ...
            if ( $method and $method =~ /CDS/ ) {
                ...
                print CDS_GFF
"$name\t$source\t$method\t$start\t$stop\t$score\t$strand\t$phase\t$field";
                $CDS{$start} = $stop;
                foreach (@group) {
...
sub movefiles {
    ...
    print "\nFinalizing...\n";
    system(
"mkdir $dir/alignments $dir/tables $dir/miscs; mv $dir/*.fna $dir/alignments/; mv $dir/*.txt $dir/tables; mv $dir/*.m $dir/miscs; mv $dir/*.*x $dir/miscs"
    );
    if ( $trees > 0 ) {
...
"raxmlHPC-PTHREADS -p 10 -T $thread -f b -m GTRGAMMAI -t $outdir/RAxML_bestTree.$name -s $outdir/$name\_all_alignment.fna -z $outdir/RAxML_bootstrap.$name\_b -w $outdir -n $name\_best 2>>$error >> $log\n\n";
    ...
    if ( $tree == 3 || $tree == 4 ) {
        print OUT
"Reconstructing phylogeny using IQ-tree after finding the best model\n";
        if ( $bsignal == 1 ) {
...
"runPAML.pl -i $dir -t $thread -r $tree -m $model -n $NSsites -s $suffix 2>>$error >> $log\n\n";
        ...
        my $parse = "parseSitePAML.pl $pamldir $NSsites 2>>$error >> $log\n\n";
        print OUT $parse;
    ...
    if ( $model == 2 ) {
        ...
        my ( $tree_name, $dirs, $tree_suffix ) =
          File::Basename::fileparse( $tree, qr/\.[^.]*/ );
        my $branch_labeled_tree =
          $pamldir . "/$tree_name" . "_BranchNumber" . "$tree_suffix";
        print OUT "\n";
        my $ps =
...
sub PickRefGenome {
    ...
    my @reffiles = grep { /\.fna$|\.fasta$|\.fa$|\.fsa$/ && -f "$refdir/$_" }
      readdir(REFDIR);
...
sub SNPsAnalysis {
    ...
    while ( my $files = readdir(DIR) ) {
        ...
        if ( $files =~ /contigs?.snps$/ ) {
            ...
            my $p =
"SNP_analysis.pl -gff $cds_gff -SNP $snp_file -format nucmer -fasta $ref_fasta -output $snp_dir 2>>$error >> $log\n\n";
            print OUT $p;
            ...
            $snp_file = $snp_dir . '/' . $files;
            my $p =
"SNP_analysis.pl -gff $cds_gff -SNP $snp_file -format vcf -fasta $ref_fasta 2>>$error -output $snp_dir >> $log\n\n";
            print OUT $p;
            if ( system($p) ) { die "Error running $p.\n"; }
        }
    }
